...
class hitDataModule(adios2DataModule):
     ...
     def __init__(self, val_dirs, seq_len: List[int] = [10, 100], data_dir: str = "./train", 
                 ...
                 adios_cfg: str = "./adios2.xml", target_dims = [32,32,32], restart_reservoir=True, n_streams: int = 3, reservoir_per_node: bool = False):

        super().__init__(data_dir, 
                 batch_size, reservoir_treshold ,
                 var_names, 
                 adios_cfg , target_dims, restart_reservoir, n_streams=n_streams, reservoir_per_node=reservoir_per_node)

        self.val_dirs = val_dirs
        self.val_data = []
        for i in range(len(self.val_dirs)):
            self.val_data.append(create_data_list(self.val_dirs[i:i+1], seq_len[1], seq_len[1]))
     ...
     def val_dataloader(self):
        ...
        for i in range(len(self.val_ds)):
            dl.append(DataLoader(self.val_ds[i], batch_size=5, num_workers=3))
