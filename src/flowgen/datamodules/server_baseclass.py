...
class adios2StreamDataset(Dataset):
    def __init__(self, data_dir: str, 
                 ...
                 engines: List, io: any, reservoir_per_node: bool = False):
        super().__init__()
        ...
        self.io = io
        self.prepare_data_per_node = reservoir_per_node
        ...
        seen = []
        unseen = []
        if reservoir_per_node:
            self.reservoir = TensorDict.load_memmap(self.data_dir+f"/reservoir_{self.rank // self.local_size}")
        else:
            self.reservoir = TensorDict.load_memmap(self.data_dir+f"/reservoir_0")
    ...
    def get_new_data(self):
        ...
        while nprocessed < self.reservoir_treshold:
            for n_engine, engine in enumerate(self.engines):
                try:
                    step_status = engine.BeginStep(mode=adios2.StepMode.Read,timeoutSeconds=0.0)
                except:
                    #SEARCH NEW SIMULATIONS
                                        if sum(self.engines_status)==0:
                                                sst_files = []
                                                while len(sst_files) < len(self.engines):
...
class adios2DataModule(L.LightningDataModule):
    def __init__(self, data_dir: str, 
                 ...
                 var_names : List[str], 
                 adios_cfg: str , target_dims : List[int], restart_reservoir: bool = True, n_streams: int = 3, reservoir_per_node: bool =  False):
        super().__init__()
        self.data_dir = data_dir
        ...
        if self.local_rank ==0:
            self.adios = adios2.ADIOS(self.adios_cfg, MPI.COMM_SELF)
            self.io     = self.adios.DeclareIO("readerIO")
            sst_files = []
            while len(sst_files) < self.n_streams:
                for root, dirs, files in os.walk(self.data_dir):
                    for file in files:
                        if file.endswith(".sst"):
                            sst_files.append(os.path.join(root, file.replace(".sst","")))
            engines = []
            for i in range(len(sst_files)):
                engines.append(self.io.Open(os.path.relpath(sst_files[i]), 
                adios2.Mode.Read, MPI.COMM_SELF))
            
            self.engines = engines
            self.engines_status = [True] * len(self.engines)
            print(f"finished adios2 init: number of engines =  {len(self.engines)}")
        else:
             self.engines = []
             self.io =  None
            ...
            while nprocessed < self.reservoir_treshold:
                for n_engine in range(len(self.engines)):
                    try:
                        ...
                        step_status = engine.BeginStep(mode=adios2.StepMode.Read,timeoutSeconds=60.0)
                    except:
                        #SEARCH NEW SIMULATIONS
                            if sum(self.engines_status)==0:
                                sst_files = []
                                    ...
                                    for root, dirs, files in os.walk(self.data_dir):
                                        for file in files:
                                            if file.endswith(".sst"):
                                                sst_files.append(os.path.join(root, file.replace(".sst","")))
                                for i in range(len(sst_files)):
                                        self.engines[n_engine] = self.io.Open(os.path.relpath(sst_files[i]), adios2.Mode.Read, MPI.COMM_SELF)
                                        self.engines_status[n_engine] = True
                    else:
                        if step_status == adios2.StepStatus.OK:
                            for i in range(len(self.var_names)):
                            ...
                            nprocessed += 1
                                    ...
                                    engine.Close()
                                    print(f'Engine {n_engine} closed!')
                                    self.engines_status[n_engine] = False
            
            del var
            reservoir.memmap(self.data_dir+f"/reservoir_{self.rank // self.local_size}")
        print("Reservoir Full!")

    def setup(self, stage: str):
        # Assign train/val datasets for use in dataloaders
        if stage == "fit":
            self.train_ds = adios2StreamDataset(self.data_dir, self.reservoir_treshold, self.var_names, self.target_dims, self.engines, self.io, self.prepare_data_per_node)
                ...
                while True:  # Continuous monitoring
                    ...
                    for filename in os.listdir(directory):
                        if filename.endswith('.sst'):
                            sst_files.append(os.path.join(directory, filename))
                            time.sleep(5)  # Extra safety delay
                            return
