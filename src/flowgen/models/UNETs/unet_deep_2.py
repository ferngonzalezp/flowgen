...
class UNet(nn.Module):
  ...
  #def __init__(self , batch_size, retain_dim=False): 
    ...
    self.PreConv = PreConv(1, 16, kernel_size=(3, 3, 3))

    # create encoder for the model  
    self.encoder1 = Encoder(16,32 ,kernel_size=(7,7,7), same_size = False)
    self.encoder2 = Encoder(32,64,kernel_size=(5,5,5), same_size = False)
    self.encoder3 = Encoder(64,128,kernel_size=(5,5,5), same_size = False)
    self.encoder4 = Encoder(128,256, kernel_size=(3,3,3), same_size = False)
    self.encoder5 = Encoder(256,512, kernel_size=(3,3,3), same_size = False) 
    self.encoder6 = Encoder(512,1024, kernel_size=(3,3,3), same_size = False) 
    
    # create Linear layer to concatenate input parameters and encoder output with same size
    self.linear1 = nn.Linear(4096, 2048)
    self.linear2 = nn.Linear(10,2048)

    # create batch normalization before bottleneck layer
    self.bn_before = nn.BatchNorm1d(4096)
    ...
    self.botleneck = Bottleneck(4096,4096)

    # create decoder for the x-component model 
    self.decoderx1 = Decoder(1024, 512, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderx2 = Decoder(512, 256, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderx3 = Decoder(256, 128, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderx4 = Decoder(128, 64, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decoderx5 = Decoder(64, 32, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decoderx6 = Decoder(32, 16,  kernel_size=(7,7,7), unit_scal_fact=False)

    # create decoder for the y-component model 
    self.decodery1 = Decoder(1024, 512, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decodery2 = Decoder(512, 256, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decodery3 = Decoder(256, 128, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decodery4 = Decoder(128, 64, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decodery5 = Decoder(64, 32, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decodery6 = Decoder(32, 16,  kernel_size=(7,7,7), unit_scal_fact=False)

    # create decoder for the z-component model 
    self.decoderz1 = Decoder(1024, 512, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderz2 = Decoder(512, 256, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderz3 = Decoder(256, 128, kernel_size=(3,3,3), unit_scal_fact=False)
    self.decoderz4 = Decoder(128, 64, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decoderz5 = Decoder(64, 32, kernel_size=(5,5,5), unit_scal_fact=False)
    self.decoderz6 = Decoder(32, 16,  kernel_size=(7,7,7), unit_scal_fact=False)
  ...
  def forward(self, input, param):
    ...
    dec_x  = torch.cat((dec_x6, input), 1)
    dec_y   = torch.cat((dec_y6, input), 1)
    dec_z  = torch.cat((dec_z6, input), 1)   
