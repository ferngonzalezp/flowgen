...
class OutputWriter:
    ...
    def __init__(self, input_reader: InputReader, unit_handler: UnitHandler, domain_information: DomainInformation,
        ...
        self.write_on_disk  = self.numerical_setup["output"]["write_on_disk"]
        ...
        if stream:
            ...
            quantities.append("time")
            self.temp_var = dict(zip(quantities, ([] for _ in quantities)))
            self.count = 0
    ...
    def send_fields(self, buffer_dictionary: Dict[str, Dict[str, jnp.ndarray]]) -> None:
         current_time = buffer_dictionary["time_control"]["current_time"]
         diff = current_time - self.next_timestamp
         if diff >= -jnp.finfo(jnp.float64).eps:
        #COMPUTE Temperature
            ...
            fields = dict()
            if all(len(self.temp_var[x]) == 0 for x in self.temp_var.keys()):
                self.engine.BeginStep()
            if self.nprocessed == 0:
                            var_id = np.zeros((self.nt))
                            self.var_id['time'] = self.io.DefineVariable('time', var_id, [*var_id.shape], [0]*len(var_id.shape), [*var_id.shape],
                                                  adios2.ConstantDims)
            self.temp_var['time'].append(current_time)
            if len(self.temp_var['time']) == self.nt:
                                self.temp_var['time'] = np.stack(self.temp_var['time'],-1)
                                self.engine.Put(self.var_id['time'], self.temp_var['time'])
                                self.count += 1
            for key in ["primes", "cons"]:
                if key in self.output_quantities.keys():
                    for quantity in self.output_quantities[key]:
                        if self.levelset_type == "FLUID-FLUID":
                            for i in range(2):
                                ...
                                buffer = self.unit_handler.dimensionalize(material_fields[key][self.quantity_index[key][quantity], i, self.nhx, self.nhy, self.nhz], quantity)
                                if self.nprocessed == 0:
                                    var_id = np.zeros((buffer.T.shape[0], buffer.T.shape[1], buffer.T.shape[2], self.nt))
                                    self.var_id[quantity] = self.io.DefineVariable(quantity, var_id, [*var_id.shape], [0]*len(var_id.shape), [*var_id.shape],
                                                  adios2.ConstantDims)
                                self.temp_var[quantity].append(buffer.T)
                                if len(self.temp_var[quantity]) == self.nt:
                                    self.temp_var[quantity] = np.stack(self.temp_var[quantity],-1)
                                    self.engine.Put(self.var_id[quantity], self.temp_var[quantity])
                                    self.count += 1
                            ...
                            if len(self.temp_var[quantity]) == self.nt:
                                ...
                                self.engine.Put(self.var_id[quantity], self.temp_var[quantity])
                                self.count += 1     
            if self.count == len(self.temp_var):
                self.engine.EndStep()
                quantities = []
                for key in ["primes", "cons"]:
                        if key in self.output_quantities.keys():
